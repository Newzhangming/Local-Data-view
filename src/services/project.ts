import { BaseResp, IdReq, PerformanceReq, ProjectDto, ProjectReq } from '@/constants/dto';
import { PerformanceType, ProjectDetailDto, ProjectReqDto } from '@/constants/project';
...
export class ProjectService extends BaseService {
  ...
  async performance(input: PerformanceReq): Promise<BaseResp<PerformanceType[]>> {
    const pageIndex = input.current || 1;
    const url = `/v1/project/performance?${this.objectToQueryString({ pageIndex, ...input })}`;
    return this.http.get<BaseResp<PerformanceType[]>>(url, this.createConfig());
  }
}
